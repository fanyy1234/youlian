...
public class MyYejiActivity extends BaseActivity implements OnClickListener {
...
////                            xRefreshView1.stopLoadMore(false);
	...
	private void record() {
		Call<ResultDto> call = ApiClient.getApiAdapter().myAchievement(MyApplication.getToken(),pageNo, BaseUrl.pageSize);
			...
			public void onResponse(Call<ResultDto> call, retrofit2.Response<ResultDto> response) {
				...
				if (response.body().getCode() == 0) {
					...
					for (int i = 0; i < length; i++) {
						...
						Long investorNumber = jsonObject.getLong("investorNumber");
						Double commission = jsonObject.getDouble("commission");
						String investorMobile = jsonObject.getString("investorMobile");
						String investorNickName = jsonObject.getString("investorNickName");
						String time = jsonObject.getString("createTime");
						Yeji yeji = new Yeji();
						yeji.setNickname(investorNickName);
						yeji.setNum(investorNumber);
						yeji.setPhone(investorMobile);
						yeji.setTicheng(commission);
						yeji.setTime(time);
						models.add(yeji);
					}
