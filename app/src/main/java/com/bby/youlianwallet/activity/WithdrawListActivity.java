...
public class WithdrawListActivity extends BaseActivity implements OnClickListener {
...
////                            xRefreshView1.stopLoadMore(false);
	...
	private void buyRecord() {
		Call<ResultDto> call = ApiClient.getApiAdapter().takeRecord(MyApplication.getToken(),pageNo, BaseUrl.pageSize);
			...
			public void onResponse(Call<ResultDto> call, retrofit2.Response<ResultDto> response) {
				...
				if (response.body().getCode() == 0) {
					...
					for (int i = 0; i < length; i++) {
						...
						int type = jsonObject.getInteger("takeType");
						String time = jsonObject.getString("takeTime");
						long num = jsonObject.getLong("takeNumber");
						int orderStatus = jsonObject.getInteger("takeStatus");
						String payAddress = jsonObject.getString("purseAddress");
						WithdrawDetail withdrawDetail = new WithdrawDetail();
						withdrawDetail.setBetaNum(num);
						withdrawDetail.setOrderStatus(orderStatus);
						withdrawDetail.setTime(time);
						withdrawDetail.setType(type);
						withdrawDetail.setPayAddress(payAddress);
						models.add(withdrawDetail);
					}
