...
public class RechargeListActivity extends BaseActivity implements OnClickListener {
...
////                            xRefreshView1.stopLoadMore(false);
	...
	private void buyRecord() {
		Call<ResultDto> call = ApiClient.getApiAdapter().buyRecord(MyApplication.getToken(),pageNo, BaseUrl.pageSize);
			...
			public void onResponse(Call<ResultDto> call, retrofit2.Response<ResultDto> response) {
				...
				if (response.body().getCode() == 0) {
					...
					for (int i = 0; i < length; i++) {
						...
						int type = jsonObject.getInteger("payType");
						String time = jsonObject.getString("ct");
						long num = jsonObject.getLong("buyNumber");
						int orderStatus = jsonObject.getInteger("orderStatus");
						String payAddress = jsonObject.getString("receivableAddress");
						RechargeDetail rechargeDetail = new RechargeDetail();
						rechargeDetail.setBetaNum(num);
						rechargeDetail.setOrderStatus(orderStatus);
						rechargeDetail.setTime(time);
						rechargeDetail.setType(type);
						rechargeDetail.setPayAddress(payAddress);
						models.add(rechargeDetail);
					}
